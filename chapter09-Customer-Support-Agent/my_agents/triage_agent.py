...
async def off_topic_guardrail(
    ...
    return GuardrailFunctionOutput(
        ...
        tripwire_triggered=result.final_output.is_off_topic, # 이게 true 이면 AI의 답변이 멈추게 된다
    )
...
def make_handoff(agent):
    ...
    return handoff(
        ...
        # 새로운 데이터가 볼 수 있는 데이터를 골라서 넘겨줄 수 있음
        input_filter=handoff_filters.remove_all_tools, # 어떤 툴을 사용했는지에 대한 기록을 지움
    )
...
triage_agent = Agent(
    ...
    #     # off_topic_guardrail,
    # ],
    # tools를 사용하면 triage_agent만 이용자와 상호작용하고 triage_agent가 tools의 기능을 활용하여 답을 줌
    # tools=[
    ...
    #     )
    # ]
    # 이렇게 handoffs를 사용하면 triage_agent가 답변할 수 없을 때 다른 agent가 이용자와 상호작용하게 됨
    handoffs=[
